...
void main() {
  group('EventParser.parseMessage', () {
    test('单事件：permission_request + taskId + title', () {
      ...
      expect(events.first.summary, 'rm -rf build');
    });
  ...
  group('NotificationSpec.from', () {
    ...
    test('error → 失败渠道/高优先级，带会话名，无摘要用兜底', () {
      ...
      expect(spec.body, '任务失败，点击打开对话');
    });

    test('completed → 完成渠道/高优先级；非白名单返回 null', () {
  ...
  group('NotificationPrefs（默认全部通知）', () {
    test('默认值：审批、完成、失败均开启', () {
      const prefs = NotificationPrefs();
      expect(prefs.approval, isTrue);
      expect(prefs.complete, isTrue);
      expect(prefs.fail, isTrue);
    });
    ...
    test('enabled 映射：审批族共用一个开关位', () {
      ...
      expect(prefs.enabled('completed'), isTrue);
      expect(prefs.enabled('error'), isTrue);
  ...
  group('TaskIndexExtractor（controller/tasks-index，跨项目全量）', () {
    ...
    test('扁平 task delta 也保留非当前会话的待审批计数', () {
      const running =
          '{"payload":{"deltas":[{"op":"task.upserted","task":{'
          '"address":{"taskId":"sess_other"},'
          '"meta":{"taskId":"sess_other","title":"另一个会话",'
          '"status":"running"},'
      ...
      const waiting =
          ...
          '"pendingInteractionSummary":{"permissionCount":1,'
          '"userInputCount":0},"pendingInteraction":{"kind":"permission",'
          '"toolName":"Write","description":"写入文件"}}}}]}}';

      final differ = StateDiffer();
      differ.apply(TaskIndexExtractor.parse(running));
      final events = differ.apply(TaskIndexExtractor.parse(waiting));
      expect(events, hasLength(1));
      expect(events.single.type, 'permission_request');
      expect(events.single.taskId, 'sess_other');
      expect(events.single.sessionTitle, '另一个会话');
    });
  ...
  group('NotificationGate（前台应用内、后台系统提醒）', () {
    ...
    test('前台 + 同设备 + 另一个会话：由应用内通知承接，不发系统通知', () {
      expect(
        NotificationGate.shouldNotify(
          appForeground: true,
          visibleDeviceId: 'd1',

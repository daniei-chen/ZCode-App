...
void main() {
  ...
  group('SessionGrouping.groupByWorkspace', () {
    test('按真实 workspacePath 分组并保持会话排序', () {
      final list = [
        sess('a', workspacePath: r'E:\repo\alpha', lastActivityAt: 30),
        sess('b', workspacePath: r'E:\repo\alpha', lastActivityAt: 20),
        sess('c', workspacePath: r'E:\repo\beta', lastActivityAt: 10),
      ];
      final groups = SessionGrouping.groupByWorkspace(list);
      expect(groups.map((g) => g.key), [r'E:\repo\alpha', r'E:\repo\beta']);
      expect(groups.first.value.map((s) => s.sessionId), ['a', 'b']);
      expect(SessionGrouping.workspaceLabel(r'E:\repo\alpha'), 'alpha');
    });

    test('缺路径时回退到工作区名，完全缺失时不丢会话', () {
      final groups = SessionGrouping.groupByWorkspace([
        sess('named', workspace: '项目 A'),
        sess('unknown'),
      ]);
      expect(groups.map((g) => g.key), ['项目 A', 'unknown']);
      expect(SessionGrouping.workspaceLabel('unknown'), '未命名工作区');
    });
  });
  ...
  group('SessionIndexNotifier', () {
    ...
    test('首见 upsert 建设备条目', () {
      ...
      expect(
        container.read(sessionIndexProvider)['d1']?['s1']?.phase,
        'running',
      );
    });
    ...
    test('removeSessions 移除指定 sessionId', () {
      ...
      expect(container.read(sessionIndexProvider)['d1']?.keys, ['s2']);
    });
    ...
    group('upsertTasks（tasks-index 全量底座，覆盖缺口修复）', () {
      ...
      test('合并：待办计数与交互细节保留既有（tasks-index 不得清掉）', () {
        ...
        notifier.upsertTasks('d1', [sess('s1', lastActivityAt: 99)]);
        final s = container.read(sessionIndexProvider)['d1']?['s1'];
        expect(s?.permissionCount, 2);
      ...
      test('session 快照时间戳推进后胜出；无 session 覆盖时 task 时间戳生效', () {
        final notifier = container.read(sessionIndexProvider.notifier);
        notifier.upsertTasks('d1', [
          sess('t1', lastActivityAt: 100, createdAt: 80),
        ]);
        expect(
        ...
        notifier.upsertAll('d1', [
          sess('t1', phase: 'running', lastActivityAt: 200),
        ]);
        expect(
          container.read(sessionIndexProvider)['d1']?['t1']?.lastActivityAt,
          200,
      ...
      test('replaceTasks：snapshot 全量替换——清空语义，残留摘除', () {
        final notifier = container.read(sessionIndexProvider.notifier);
        notifier.upsertTasks('d1', [
          sess('t_old', title: '旧', lastActivityAt: 1),
        ]);
        notifier.upsertAll('d1', [
          sess('s_keep', phase: 'running', lastActivityAt: 5),
        ]);
        ...
        expect(container.read(sessionIndexProvider)['d1']?.keys, ['s_keep']);
      });

      test(
        'replaceTasks(preservePinned)：bootstrap 扁平视图无 pinned——既有置顶保留（评审 r10）',
        () {
          final notifier = container.read(sessionIndexProvider.notifier);
          notifier.upsertTasks('d1', [
      ...
      test(
        'replaceTasks 不带 preservePinned：snapshot 权威覆盖置顶（false 就该是 false）',
        () {
          final notifier = container.read(sessionIndexProvider.notifier);
          notifier.upsertTasks('d1', [
            sess('t_unpin', pinned: true, lastActivityAt: 10),
          ]);

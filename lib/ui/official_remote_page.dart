...
''';
  ...
  /// 手机端专用：官方页顶栏"主题键"左侧注入一个"应用设置"齿轮（克隆
  /// 主题按钮保证样式一致，图标换成齿轮），点击后打开官方页自身的
  /// 设置界面。平板不注入（宽度大，页面自带布局已够用）。
  static const _webNavExtrasScript = r'''
(function () {
  if (window.__zrNavExtrasInstalled) return;
  var tries = 0;
  function findTheme() {
    ...
    for (var i = 0; i < nodes.length; i++) {
      ...
      if (rect.width <= 0 || rect.top > 140 || rect.height > 80) continue;
      if (label.indexOf('主题') >= 0 || label.indexOf('theme') >= 0 ||
          label.indexOf('外观') >= 0 || label.indexOf('appearance') >= 0) {
        return n;
      }
    }
    return null;
  ...
  function findSettings() {
    var selectors = [
      ...
      '[data-testid*="settings"]', '[data-testid*="Setting"]'
    ];
    for (var i = 0; i < selectors.length; i++) {
      var els = document.querySelectorAll(selectors[i]);
      for (var j = 0; j < els.length; j++) {
        var n = els[j];
        if (n.closest('[data-zr-injected]')) continue;
        var rect = n.getBoundingClientRect();
  ...
  window.__zrOpenSettings = function () {
    var s = findSettings();
  ...
  function inject() {
    ...
    window.__zrNavExtrasInstalled = true;
    var gear = theme.cloneNode(true);
    gear.setAttribute('aria-label', '应用设置');
    gear.setAttribute('data-zr-injected', '1');
    gear.removeAttribute('disabled');
    var svgs = gear.querySelectorAll('svg');
    for (var j = 0; j < svgs.length; j++) svgs[j].remove();
    gear.insertAdjacentHTML('afterbegin',
    ...
    gear.addEventListener('click', function (ev) {
      ev.preventDefault();
      ev.stopPropagation();
      window.__zrOpenSettings();
    }, true);
    theme.parentNode.insertBefore(gear, theme);
    return true;
  }
  function attempt() {
    if (inject()) return;
...
''';
  ...
  /// 注入手机端导航增强：主题键左侧的设置齿轮 + 页面内设置跳转。
  Future<void> _injectWebNavExtras() async {
    try {
      await _controller?.evaluateJavascript(source: _webNavExtrasScript);
    } catch (_) {}
  }

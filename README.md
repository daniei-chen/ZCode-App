...
ZCode App（当前版本 1.0.4）面向需要在手机或平板上管理 ZCode 桌面任务的用户。它通过 ZCode 桌面端的移动远程控制功能接入电脑：扫码或粘贴控制链接，命名并保存设备，之后即可从设备页快速进入最近使用的设备和会话。
...
- **通知音效**：应用内和应用外通知统一使用系统默认通知声音，不提供自定义声音选择。
...
- **更新入口**：设备页提供检查更新和 GitHub 仓库入口；发现新版后直接抓取 Release 的 APK，在应用内显示进度并交给系统安装，不跳转浏览器。
...
lib/
  ...
  ui/           设备、WebView 会话入口、通知、工作台和设置页面
